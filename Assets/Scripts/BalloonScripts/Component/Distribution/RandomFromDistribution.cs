...
//uses great script from Unity Asset Store published by Nathan Daly
//Distribution
public static class RandomFromDistribution  {

    //Lognormal Distribution
    //simulation based on http://blogs.sas.com/content/iml/2014/06/04/simulate-lognormal-data-with-specified-mean-and-variance.html
    public static float RandomLogNormalDistribution(float mean, float std_dev)
    {
        float euler = 2.71828182845904523536f;
        float var = std_dev * std_dev;
        float sqrMean = mean * mean;

        float phi = Mathf.Sqrt(var +  sqrMean);
        float meanLog = Mathf.Log(sqrMean / phi, euler);
        float std_devLog = Mathf.Sqrt(Mathf.Log((phi * phi) / sqrMean,euler));

        float x = RandomNormalDistribution(meanLog,std_devLog);

        //if x is normal distributed then exp(x) is lognormal distributed
        return Mathf.Exp(x);
    }
